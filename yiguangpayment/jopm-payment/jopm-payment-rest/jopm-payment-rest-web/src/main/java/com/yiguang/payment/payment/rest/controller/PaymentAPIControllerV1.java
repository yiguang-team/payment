...
public class PaymentAPIControllerV1
{
			...
			@RequestParam(value = "sign", defaultValue = "") String sign, ServletResponse response) throws IOException
	{
		...
		try
		{
			...
			SmsAgentPaymentService woIreadPaymentService = (SmsAgentPaymentService)SpringUtils.getBean("woIreadPaymentService");
		...
		httpResponse.getWriter().write(
				"<?xml version=\"1.0\" encoding=\"utf-8\"?><response><error>" + error + "</error>" + sms
						+ "</response>");
	}
