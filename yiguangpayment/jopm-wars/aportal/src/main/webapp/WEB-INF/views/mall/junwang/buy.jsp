					...
					<form action="${ctx}/mall/YGPAY/placeOrder" method="post"
						name="form1">
						 <input
							...
							id="channelId" value="3" type="hidden" /> 
						<table width="582" height="236" border="0">
							...
							<tr>
								<td width="300" align="right">计费类型： <br /></td>
								<td width="537"><span id="chargingTypeList">
									<select name="chargingType" id="chargingType" onchange="pointList()" class="form-control fl"
										style="height: 30px; width: 375px;">
										<c:choose>
...
function pointList(){
	...
	$.post('${ctx}/mall/YGPAY/pointList',{merchantId:merchantId,channelId:channelId,chargingType:chargingType},function(response){
...
function dosubmit()
{
	...
	if(chargingType == 1 && account== ""){
		...
		$.post('${ctx}/mall/YGPAY/checkAccount',{account:account},function(response){
			if (response == 0)
			{
	</c:if>
				$("#username_msg").html('');
						...
						if (response == 0)
						{
							$("#mobile_msg").html('');
							 var checkCode = $("#checkCode").val();
								$.post("${ctx}/mall/YGPAY/checkCode",{checkCode:checkCode},function(msg){
									if(msg!=0){
										alert("验证码错误！");
										return;
									}
