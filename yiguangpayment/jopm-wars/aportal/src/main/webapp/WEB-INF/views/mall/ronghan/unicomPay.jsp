				...
				<tr>
					<td>
						<div class="jf" style="float: left; margin-left: 50px">
							<a
								href="${ctx}/mall/ronghan/queryOrder?orderid=${orderid}&chargingType=${chargingType}&productId=${productId}&sign=${querySign}">查看订单</a>
						</div>
...
function SetRemainStatus() {
            if (statusCurCount == 0) {                
				window.location.href="${ctx}/mall/ronghan/queryOrder?orderid=${orderid}&chargingType=${chargingType}&sign=${querySign}";
            }
            else {
                curCount--;
                 $("#second").val(statusCurCount);
