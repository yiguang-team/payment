...
<body class="no-skin">
												...
												<div class="step-pane active" data-step="1">
													...
													<form class="form-horizontal" id="validation-form"
														...
														<input type="hidden" name="id" id="id"
															<c:if test="${merchant.id != null}">value="${merchant.id}"</c:if>
															<c:if test="${merchant.id == null}">value="0"</c:if> />
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="name" name="name"
																		...
																		<c:if test="${merchant.name != null}">value="${merchant.name}"</c:if> />
																</div>
															</div>
														</div>
														...
														<div class="form-group">
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<c:choose>
																		<c:when
																			test="${fn:length(channelMerchantRelationList) > 0}">
																			<c:forEach items="${channelMerchantRelationList}"
																				var="option">
																				<div class="checkbox">
																			...
																			<c:when test="${fn:length(statusList) > 0}">
																				<c:forEach items="${statusList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == merchant.status}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
														...
														<div class="form-group">
															<label
																class="control-label col-xs-12 col-sm-3 no-padding-right"
																for="adminUser">操作员:</label>

															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<select name="adminUser" id="adminUser" class="select2"
																		data-placeholder="请选择...">
																		<c:choose>
																			<c:when test="${fn:length(userList) > 0}">
																				<c:forEach items="${userList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == merchant.adminUser}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="key" name="key"
																		class="col-xs-12 col-sm-5"
																		<c:if test="${merchant.key != null}">value="${merchant.key}"</c:if> />
																</div>
															</div>
														</div>
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="notifyUrl" name="notifyUrl"
																		class="col-xs-12 col-sm-5"
																		<c:if test="${merchant.notifyUrl != null}">value="${merchant.notifyUrl}"</c:if> />
																</div>
															</div>
														</div>
														...
														<button class="btn btn-sm btn-primary no-border"
															onclick="saveMerchant()">保存</button>
													</form>
												</div>
											</div>
											<!-- /section:plugins/fuelux.wizard.container -->
	...
	<script type="text/javascript">
		window.jQuery
				|| document
						.write("<script src='${ctx}/template/ace/assets/js/jquery.js'>"
								+ "<"+"/script>");
	</script>
...
<![endif]-->
	<script type="text/javascript">
		if ('ontouchstart' in document.documentElement)
			document
					.write("<script src='${ctx}/template/ace/assets/js/jquery.mobile.custom.js'>"
							+ "<"+"/script>");
	</script>
	...
	<script type="text/javascript">
		...
		jQuery.validator.addMethod("englishUpperCase",
									...
									$
											.ajax({
												data : {
													...
													channelMerchantRelationIDs:str
												},
												url : "${ctx}/payment/management/merchant/saveMerchant",
												type : 'post',
												cache : false,
												async : true,
												dataType : 'json',
