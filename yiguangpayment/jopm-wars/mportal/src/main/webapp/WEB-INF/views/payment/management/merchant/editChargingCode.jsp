...
<title>翼光联运支付管理系统-编辑商户计费产品</title>
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<select name="merchantId" id="merchantId"
																		class="select2" data-placeholder="请选择...">
																		<c:choose>
																			<c:when test="${fn:length(merchantList) > 0}">
																				<c:forEach items="${merchantList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == chargingCode.merchantId}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="name" name="name"
																		...
																		<c:if test="${chargingCode.name != null}">value="${chargingCode.name}"</c:if> />
																</div>
															</div>
														</div>
														<div class="form-group">
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/payment/management/merchant/saveChargingCode",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
