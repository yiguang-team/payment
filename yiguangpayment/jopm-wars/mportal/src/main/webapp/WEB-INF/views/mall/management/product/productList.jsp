...
<body class="no-skin">
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/mall/management/product/productList">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									<span class="labeltxt fl">产品名称：</span> <input
									...
									</select> <span class="labeltxt fl">类型：</span> <select name="type"
										id="type" class="form-control fl"
										style="width: 150px; float: left;">
										<c:choose>
													...
													<div class="col-xs-6">
														<div class="btns">
															<button class="btn btn-sm btn-primary no-border"
																onclick="createProduct()">创建产品</button>
													...
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="product"
															...
															<tr>
																...
																<td>${product.name}</td>
																<td>${product.merchantLabel}</td>
																<td>${product.typeLabel}</td>

																<td><span
																	<c:if test="${product.status == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${product.status == 1}">class="label label-sm label-success"</c:if>>
																		${product.statusLabel} </span></td>
																<td><div class="edit" style="float: left;">
																		<c:if test="${product.status == 0}">
																			<a href="javascript:;"
																				onclick="updateProduct(1,${product.id})">[开启]</a>
																		</c:if>
																		<c:if test="${product.status == 1}">
																			<a href="javascript:;"
																				onclick="updateProduct(0,${product.id})">[关闭]</a>
																		</c:if>
																		<a href="javascript:;"
																			onclick="editProduct(${product.id},'${product.name}')">[编辑]</a>
																		<!--<a href="javascript:;" onclick="deleteProduct(${product.id})">[删除]</a>-->
																		<a href="javascript:;"
																			onclick="pointManage(${product.id})">[计费点管理]</a>
																	</div></td>
															</tr>
														</c:forEach>
													</c:when>
...
function createProduct(){
	...
	parent.addTab('新建产品','${ctx}/mall/management/product/createProduct');
...
function updateProduct(status,productId){
	...
	bootbox.confirm("确定要"+actionStr+"此产品吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/mall/management/product/updateProductStatus",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/mall/management/product/deleteProduct",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function pointManage(productId){
		parent.addTab('计费点管理','${ctx}/mall/management/point/pointList?productId='+productId);
}

function editProduct(productId,name){
	parent.addTab('编辑产品-'+name,'${ctx}/mall/management/product/editProduct?id='+productId);
}
</script>
</html>
