...
<body class="no-skin">
	...
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/payment/management/channel/channelList">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									<span class="labeltxt fl">渠道名称：</span> <input
													...
													<div class="col-xs-6">
														<div class="btns">
															<button class="btn btn-sm btn-primary no-border"
																onclick="createChannel()">创建支付渠道</button>
														</div>
													</div>
													<div class="col-xs-6">
											...
											<tbody>
												<c:choose>
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="channel"
															varStatus="status">
															<tr>
																...
																<td>${channel.name}</td>
																<td><span
																	<c:if test="${channel.status == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${channel.status == 1}">class="label label-sm label-success"</c:if>>
																		${channel.statusLabel} </span></td>

																<td><div class="edit" style="float: left;">
																		<c:if test="${channel.status == 0}">
																			<a href="javascript:;"
																				onclick="updateChannelStatus(1,'${channel.id}')">[开启]</a>
																		</c:if>
																		<c:if test="${channel.status == 1}">
																			<a href="javascript:;"
																				onclick="updateChannelStatus(0,'${channel.id}')">[关闭]</a>
																		</c:if>
																		<a href="javascript:;"
																			onclick="editChannel('${channel.id}')">[编辑]</a>
																		<!--
								<a href="javascript:;" onclick="deleteChannel('${channel.id}')">[删除]</a>
								-->
																	</div></td>
															</tr>
														</c:forEach>
...
function createChannel(){
	...
	{	
		parent.addTab('新建渠道','${ctx}/payment/management/channel/createChannel');
	}

}

function updateChannelStatus(status,id){
	var actionStr = "";
	if(status == 0){
		//关闭
		actionStr = "关闭";
	...
	bootbox.confirm("确定要"+actionStr+"此渠道吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/payment/management/channel/updateChannelStatus",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/payment/management/channel/deleteChannel",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function editChannel(id){
	parent.addTab('编辑渠道','${ctx}/payment/management/channel/editChannel?id='+id);
}
