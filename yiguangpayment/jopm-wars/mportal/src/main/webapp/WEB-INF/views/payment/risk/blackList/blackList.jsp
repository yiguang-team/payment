...
<body class="no-skin">
	...
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/payment/risk/blackList/blackList">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									<span class="labeltxt fl">值：</span> <input class="fl form-text"
										id="form-field-icon-1" type="text" id="value" name="value"
										value="${value}"> <span class="labeltxt fl">类型：</span>
									<select name="type" id="type" class="form-control fl"
										style="width: 150px; float: left;">
										<c:choose>
											<c:when test="${fn:length(typeList) > 0}">
												<c:forEach items="${typeList}" var="option">
													<option value="${option.value}"
														<c:if test="${option.value == type}">selected="true"</c:if>>${option.text}</option>
												</c:forEach>
											</c:when>
											<c:otherwise>
												<option value=""></option>
													...
													<div class="col-xs-6">
														<div class="btns">
															<button class="btn btn-sm btn-primary no-border"
																onclick="createBlackList()">创建黑名单</button>
											...
											<tbody>
												<c:choose>
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="blackList"
															varStatus="status">
															<tr>
																<td class="center">${(page-1)*pageSize+status.index+1}</td>
																<td>${blackList.value}</td>
																<td>${blackList.typeLabel}</td>
																<td><span
																	<c:if test="${blackList.status == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${blackList.status == 1}">class="label label-sm label-success"</c:if>>
																		${blackList.statusLabel} </span></td>
																<td><div class="edit" style="float: left;">
																		<c:if test="${blackList.status == 0}">
																			<a href="javascript:;"
																				onclick="updateBlackListStatus(1,${blackList.id})">[开启]</a>
																		</c:if>
																		<c:if test="${blackList.status == 1}">
																			<a href="javascript:;"
																				onclick="updateBlackListStatus(0,${blackList.id})">[关闭]</a>
																		</c:if>
																		<a href="javascript:;"
																			onclick="editBlackList('${blackList.id}')">[编辑]</a>
																		<a href="javascript:;" onclick="deleteBlackList(${blackList.id})">[删除]</a>
																	</div></td>
															</tr>
														</c:forEach>
													</c:when>
...
function createBlackList(){
	...
	{	
		parent.addTab('新建黑名单规则','${ctx}/payment/risk/blackList/createBlackList');
	}

}

function updateBlackListStatus(status,id){
	var actionStr = "";
	if(status == 0){
		//关闭
		actionStr = "关闭";
	...
	bootbox.confirm("确定要"+actionStr+"此渠道限量规则吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/payment/risk/blackList/updateBlackListStatus",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/payment/risk/blackList/deleteBlackList",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function editBlackList(id){
	parent.addTab('编辑黑名单规则','${ctx}/payment/risk/blackList/editBlackList?id='+id);
}
</script>
</html>