...
<body class="no-skin">
	...
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/payment/risk/basicRule/basicRuleList">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
										...
										<span class="labeltxt fl">限时方式：</span> 
										<select name="timeType" id="timeType"  class="form-control fl" style="width: 150px; float: left;">
											<c:choose>
										...
										<span class="labeltxt fl">时间单位：</span>
										<select name="timeUnit" id="timeUnit"  class="form-control fl" style="width: 150px; float: left;">
											<c:choose>
										...
										<span class="labeltxt fl">限量方式：</span>
										...
										<span class="labeltxt fl">限制量值：</span> 
										
										<input class="fl form-text" id="form-field-icon-1" type="text" id="volume" name="volume" value="${volume}"> 
										
										<span class="labeltxt fl">规&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;则：</span>
									...
									<br/><br/>
									 <span class="labeltxt fl">号&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;码：</span>
									 <input class="fl form-text" id="form-field-icon-1" type="text" id="mobile" name="mobile" value="${mobile}"> 
									  <span class="labeltxt fl">请求I&nbsp;&nbsp;&nbsp;&nbsp;P：</span>
									 <input class="fl form-text" id="form-field-icon-1" type="text" id="ip" name="ip" value="${ip}"> 
									  <span class="labeltxt fl">账&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;号：</span>
									 <input class="fl form-text" id="form-field-icon-1" type="text" id="username" name="username" value="${username}"> 
									<span class="labeltxt fl">状&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;态：</span>
									...
									<br/><br/>
									 <span class="labeltxt fl">渠&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;道：</span>
									...
									<select name="provinceId" id="provinceId" class="form-control fl"
										style="width: 150px; float: left;">
									...
									<span class="labeltxt fl">城&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;市：</span>
										...
										<c:choose>
											<c:when test="${fn:length(pointList) > 0}">
												<c:forEach items="${pointList}" var="option">
													<option value="${option.value}"
														<c:if test="${option.value == pointId}">selected="true"</c:if>>${option.text}</option>
												</c:forEach>
											</c:when>
											<c:otherwise>
												<option value=""></option>
													...
													<div class="col-xs-6">
														<div class="btns">
															<button class="btn btn-sm btn-primary no-border"
																onclick="createBasicRule()">创建基本限制规则</button>
											...
											<tbody>
												<c:choose>
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="BasicRule"
															varStatus="status">
															<tr>
																...
																<td><c:if test="${BasicRule.ip != '-8' && BasicRule.ip != '-9'}">${BasicRule.ip}</c:if><c:if test="${BasicRule.ip == '-8'}">当前</c:if></td>
																<td><c:if test="${BasicRule.username != '-8' && BasicRule.username != '-9'}">${BasicRule.username}</c:if><c:if test="${BasicRule.username == '-8'}">当前</c:if></td>
																<td>${BasicRule.channelLabel}</td>
																<td>${BasicRule.provinceLabel}</td>
																<td>${BasicRule.cityLabel}</td>
																<td>${BasicRule.merchantLabel}</td>
																<td>${BasicRule.productLabel}</td>
																<td>${BasicRule.pointLabel}</td>

																<td><span
																	<c:if test="${BasicRule.status == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${BasicRule.status == 1}">class="label label-sm label-success"</c:if>>
																		${BasicRule.statusLabel} </span></td>
																<td><div class="edit" style="float: left;">
																		<c:if test="${BasicRule.status == 0}">
																			<a href="javascript:;"
																				onclick="updateBasicRuleStatus(1,${BasicRule.id})">[开启]</a>
																		</c:if>
																		<c:if test="${BasicRule.status == 1}">
																			<a href="javascript:;"
																				onclick="updateBasicRuleStatus(0,${BasicRule.id})">[关闭]</a>
																		</c:if>
																		<a href="javascript:;"
																			onclick="editBasicRule('${BasicRule.id}')">[编辑]</a>
																		<a href="javascript:;" onclick="deleteBasicRule(${BasicRule.id})">[删除]</a>
																	</div></td>
															</tr>
														</c:forEach>
													</c:when>
													<c:otherwise>
														<tr>
															<td colspan="17" class="center">没数据</td>
														</tr>
													</c:otherwise>
												</c:choose>
											</tbody>
...
function createBasicRule(){
	...
	{	
		parent.addTab('新建基础规则','${ctx}/payment/risk/basicRule/createBasicRule');
	}

}

function updateBasicRuleStatus(status,id){
	var actionStr = "";
	if(status == 0){
		//关闭
		actionStr = "关闭";
	...
	bootbox.confirm("确定要"+actionStr+"此基础规则吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/payment/risk/basicRule/updateBasicRuleStatus",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function deleteBasicRule(basicRuleId){
	bootbox.confirm("确定要删除此规则吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/payment/risk/basicRule/deleteBasicRule",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function editBasicRule(id){
	parent.addTab('编辑基础规则'+id,'${ctx}/payment/risk/basicRule/editBasicRule?id='+id);
}
</script>
</html>