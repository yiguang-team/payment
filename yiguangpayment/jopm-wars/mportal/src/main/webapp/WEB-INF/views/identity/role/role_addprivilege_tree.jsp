			...
			<div class="main-content-inner">
				...
				<div class="page-content">
						...
						<input type="hidden" name="id" id="id" value="${id}" /> <input
							type="hidden" name="privilegeidstr" id="privilegeidstr" value="" />
						...
						<div class="line_bar">
							<button class="btn btn-sm btn-primary no-border" onclick="addprivilege()">保存</button>
						</div>
...
function addprivilege(){
	var PrivilegeId=selectcheckbox();
		...
		function(result) {
			if(result) {
				$.ajax({  
					data:{
							id:$("#id").val(),
							privilegeidstr:$("#privilegeidstr").val()
						 },
					url : "${ctx}/identity/role/saveRolePrivilegeTree",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function selectcheckbox(){
    ...
    for (var i = 0; i < nodes.length; i++) {
        if(Menuid.length==0)PrivilegeId+=nodes[i].id;
        else PrivilegeId+="|"+nodes[i].id;
    }
    $("#privilegeidstr").val(PrivilegeId);
    return PrivilegeId;
}
</script>
</html>
