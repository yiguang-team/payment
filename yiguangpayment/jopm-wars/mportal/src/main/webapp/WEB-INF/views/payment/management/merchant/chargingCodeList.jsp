...
<body class="no-skin">
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/payment/management/merchant/chargingCodeList">
			...
			<div class="main-content">
				<div class="main-content-inner">
					...
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							<li><i class="ace-icon fa fa-home home-icon"></i> 支付管理<span
								class="noshow jt">></span></li>
							<li class="active">商户计费编码</li>
						</ul>
						<!-- /.breadcrumb -->
					</div>
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									<span class="labeltxt fl">名称：</span> <input
										class="fl form-text" id="form-field-icon-1" type="text"
										id="name" name="name" value="${name}"> <span
										class="labeltxt fl">商户：</span> <select name="merchantId"
										id="merchantId" class="form-control fl"
													...
													<div class="col-xs-6">
														<div class="btns">
															<button class="btn btn-sm btn-primary no-border"
																onclick="createChargingCode()">创建商户计费编码</button>
											...
											<tbody>
												...
												<c:choose>
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="chargingCode"
															varStatus="status">
															<tr>
																<td class="center">${(page-1)*pageSize+status.index+1}</td>
																<td>${chargingCode.merchantLabel}</td>
																<td>${chargingCode.chargingCode}</td>
																<td>${chargingCode.name}</td>
																<td>${chargingCode.chargingAmount}</td>
																<td><span
																	<c:if test="${chargingCode.status == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${chargingCode.status == 1}">class="label label-sm label-success"</c:if>>
																		${chargingCode.statusLabel} </span></td>

																<td><div class="edit" style="float: left;">
																		<c:if test="${chargingCode.status == 0}">
																			<a href="javascript:;"
																				onclick="updateChargingCodeStatus(1,${chargingCode.id})">[开启]</a>
																		</c:if>
																		<c:if test="${chargingCode.status == 1}">
																			<a href="javascript:;"
																				onclick="updateChargingCodeStatus(0,${chargingCode.id})">[关闭]</a>
																		</c:if>

																		<a href="javascript:;"
																			onclick="editChargingCode(${chargingCode.id},'${chargingCode.chargingAmount}')">[编辑]</a>
																		<!--<a href="javascript:;" onclick="deleteChargingCode(${chargingCode.id})">[删除]</a>-->
																	</div></td>
															</tr>
														</c:forEach>
													</c:when>
...
function createChargingCode(){

	parent.addTab('新建商户计费编码','${ctx}/payment/management/merchant/editChargingCode');

}

function updateChargingCodeStatus(status,id){
	var actionStr = "";
	if(status == 0){
		//关闭
		actionStr = "关闭";
	...
	bootbox.confirm("确定要"+actionStr+"此商户计费编码吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/payment/management/merchant/updateChargingCodeStatus",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function deleteChargingCode(id){
	bootbox.confirm("确定要删除此商户计费编码吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/payment/management/merchant/deleteChargingCode",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function editChargingCode(id,name){
	parent.addTab('编辑商户计费编码-'+name,'${ctx}/payment/management/merchant/editChargingCode?id='+id);
}
</script>
</html>
