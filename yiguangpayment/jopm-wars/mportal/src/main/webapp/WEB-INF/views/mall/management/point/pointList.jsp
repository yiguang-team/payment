...
<body class="no-skin">
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/mall/management/point/pointList">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									</select> <br /> <br /> <span class="labeltxt fl">计费类型：</span> <select
									...
									</select> <span class="labeltxt fl">开通渠道：</span> <select
										name="channelId" id="channelId" class="form-control fl"
										style="width: 150px; float: left;">
										<c:choose>
									...
									</select> <span class="labeltxt fl">状&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;态：</span>
													...
													<div class="col-xs-6">
														<div class="btns">
															<button class="btn btn-sm btn-primary no-border"
																onclick="createPoint()">创建计费点</button>
											...
											<tbody>
												...
												<c:choose>
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="point" varStatus="status">
															<tr>
																<td class="center">${(page-1)*pageSize+status.index+1}</td>
																<td>${point.name}</td>
																<td>${point.merchantLabel}</td>
																<td>${point.productLabel}</td>
																<td>${point.chargingCode}</td>
																<td>${point.chargingTypeLabel}</td>
																<td>${point.inStock}</td>
																<td>${point.provinceLabel}</td>
																<td>${point.cityLabel}</td>
																<td>${point.faceAmount}</td>
																<td>${point.payAmount}</td>
																<td>${point.deliveryAmount}</td>
																<td><span
																	<c:if test="${point.status == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${point.status == 1}">class="label label-sm label-success"</c:if>>
																		${point.statusLabel} </span></td>
																<td><div class="edit" style="float: left;">
																		<c:if test="${point.status == 0}">
																			<a href="javascript:;"
																				onclick="updatePoint(1,${point.id})">[开启]</a>
																		</c:if>
																		<c:if test="${point.status == 1}">
																			<a href="javascript:;"
																				onclick="updatePoint(0,${point.id})">[关闭]</a>
																		</c:if>
																		<a href="javascript:;"
																			onclick="editPoint(${point.id},'${point.name}')">[编辑]</a>
																		<a href="javascript:;" onclick="deletePoint(${point.id})">[删除]</a>
													...
													<c:otherwise>
														<tr>
															<td colspan="12" class="center">没数据</td>
														</tr>
													</c:otherwise>
												</c:choose>
											</tbody>
...
function createPoint(){
	...
	parent.addTab('新建计费点','${ctx}/mall/management/point/createPoint');
...
function updatePoint(status,pointId){
	var actionStr = "";
	if(status == 0){
		//关闭
		actionStr = "关闭";
	...
	bootbox.confirm("确定要"+actionStr+"此计费点吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/mall/management/point/updatePointStatus",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/mall/management/point/deletePoint",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function queryPointList(pointId){
		window.location.href="${ctx}/mall/management/point/pointList?pointId="+pointId;
}

function editPoint(pointId,name){
	parent.addTab('编辑计费点-'+name,'${ctx}/mall/management/point/editPoint?id='+pointId);
}
</script>
</html>
