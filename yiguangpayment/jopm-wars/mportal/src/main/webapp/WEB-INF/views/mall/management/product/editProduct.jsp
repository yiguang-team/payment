												...
												<div class="step-pane active" data-step="1">
													...
													<form class="form-horizontal" id="validation-form"
														...
														<input type="hidden" name="id" id="id"
															<c:if test="${product.id != null}">value="${product.id}"</c:if>
															<c:if test="${product.id == null}">value="0"</c:if> />
																			...
																			<c:when test="${fn:length(merchantList) > 0}">
																				<c:forEach items="${merchantList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == product.merchantId}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
																		...
																		<c:choose>
																			<c:when test="${fn:length(typeList) > 0}">
																				<c:forEach items="${typeList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == product.type}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="name" name="name"
																		...
																		<c:if test="${product.name != null}">value="${product.name}"</c:if> />
																</div>
															</div>
														</div>
														<div class="form-group">
																		...
																		<c:choose>
																			<c:when test="${fn:length(statusList) > 0}">
																				<c:forEach items="${statusList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == product.status}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/mall/management/product/saveProduct",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
