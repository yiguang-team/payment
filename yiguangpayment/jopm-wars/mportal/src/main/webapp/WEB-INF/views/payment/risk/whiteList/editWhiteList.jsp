...
<title>翼光联运支付管理系统-创建白名单规则</title>
												...
												<div class="step-pane active" data-step="1">
													...
													<form class="form-horizontal" id="validation-form"
														...
														<input type="hidden" name="id" id="id"
															<c:if test="${whiteList.id != null}">value="${whiteList.id}"</c:if>
															<c:if test="${whiteList.id == null}">value="0"</c:if>
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="value" name="value"
																		...
																		<c:if test="${whiteList.value != null}">value="${whiteList.value}"</c:if> />
																</div>
															</div>
														</div>
																			...
																			<c:when test="${fn:length(typeList) > 0}">
																				<c:forEach items="${typeList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == whiteList.type}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
																		...
																		<c:choose>
																			<c:when test="${fn:length(statusList) > 0}">
																				<c:forEach items="${statusList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == whiteList.status}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
														...
														<button class="btn btn-sm btn-primary no-border"
															onclick="saveWhiteList()">保存</button>
													</form>
												</div>
											</div>
											<!-- /section:plugins/fuelux.wizard.container -->
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/payment/risk/whiteList/saveWhiteList",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
