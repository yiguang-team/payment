...
<body class="no-skin">
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/depot/depot/pickUpRecordList">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									<span class="labeltxt fl">交易订单号：</span> <input
										class="fl form-text" type="text" id="orderId" name="orderId"
										value="${orderId}"> <span class="labeltxt fl">计费点名称：</span>
									<select name="chargingPointId" id="chargingPointId"
										...
										name="extractUser" id="extractUser" class="form-control fl"
										style="width: 150px; float: left;">
										<c:choose>
											<c:when test="${fn:length(merchantList) > 0}">
												<c:forEach items="${merchantList}" var="option">
													<option value="${option.value}"
														<c:if test="${option.value == extractUser}">selected="true"</c:if>>${option.text}</option>
												</c:forEach>
											</c:when>
											<c:otherwise>
												<option value=""></option>
													...
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="record"
															...
															<tr>
																...
																<td>${record.chargingPointLabel}</td>
																<td>${record.orderId}</td>
																<td>${record.payAmount}</td>
																<td>${record.extractCount}</td>
																<td>${record.extractUser}</td>
																<td>${record.extractType}</td>
																<td><fmt:formatDate value="${record.requestTime}"
