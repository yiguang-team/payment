...
<title>翼光联运支付管理系统-创建商户互斥规则</title>
												...
												<div class="step-pane active" data-step="1">
													...
													<form class="form-horizontal" id="validation-form"
														...
														<input type="hidden" name="id" id="id"
															<c:if test="${merchantRejection.id != null}">value="${merchantRejection.id}"</c:if>
															<c:if test="${merchantRejection.id == null}">value="0"</c:if>
															class="ipt" maxlength="20" />

														<div class="form-group">
															<label
																class="control-label col-xs-12 col-sm-3 no-padding-right"
																for="merchantA">商户:</label>

															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<select name="merchantA" id="merchantA" class="select2"
																		data-placeholder="请选择...">
																		<c:choose>
																			<c:when test="${fn:length(merchantList) > 0}">
																				<c:forEach items="${merchantList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == merchantRejection.merchantA}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
														...
														<div class="form-group">
															<label
																class="control-label col-xs-12 col-sm-3 no-padding-right"
																for="merchantB">互斥商户:</label>

															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<select name="merchantB" id="merchantB" class="select2"
																		data-placeholder="请选择...">
																		<c:choose>
																			<c:when test="${fn:length(merchantList) > 0}">
																				<c:forEach items="${merchantList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == merchantRejection.merchantB}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
																		...
																		<c:choose>
																			<c:when test="${fn:length(statusList) > 0}">
																				<c:forEach items="${statusList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == merchantRejection.status}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
														...
														<button class="btn btn-sm btn-primary no-border"
															onclick="saveMerchantRejection()">保存</button>
													</form>
												</div>
											</div>
											<!-- /section:plugins/fuelux.wizard.container -->
...
<script type="text/javascript">
function saveMerchantRejection(){
		...
		function(result) {
			if(result) {
				$.ajax({  
					data:{
							id:$("#id").val(),
							merchantA:$("#merchantA").val(),
							merchantB:$("#merchantB").val(),
							status:$("#status").val()
						 },
					url : "${ctx}/payment/risk/merchantRejection/saveMerchantRejection",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
