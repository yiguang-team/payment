...
<title>翼光联运支付管理系统-设置操作员角色</title>
												...
												<div class="step-pane active" data-step="1">
													...
													<form class="form-horizontal" id="validation-form"
														...
														<input type="hidden" name="id" id="id"
															<c:if test="${roleUser.id != null}">value="${roleUser.id}"</c:if>
															<c:if test="${roleUser.id == null}">value="0"</c:if> />
																	...
																	<c:choose>
																		<c:when test="${fn:length(roleList) > 0}">
																			<c:forEach items="${roleList}" var="option">
																				<div class="checkbox">
																					<label> <input name="roleCheckbox"
																						id="roleCheckbox" type="checkbox" class="ace"
																						value="${option.value}"
																						<c:forEach items="${checkedlist}" var="titem"><c:if test="${option.value == titem.roleId}">checked</c:if></c:forEach> /><span
														...
														<button class="btn btn-sm btn-primary no-border"
															onclick="saveRoleUser()">保存</button>
													</form>
												</div>
											</div>
											<!-- /section:plugins/fuelux.wizard.container -->
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/identity/user/saveRoleUser",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
