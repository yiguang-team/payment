												...
												<div class="step-pane active" data-step="1">
													...
													<form class="form-horizontal" id="validation-form"
														...
														<input type="hidden" name="id" id="id"
															<c:if test="${channelParms.id != null}">value="${channelParms.id}"</c:if>
															<c:if test="${channelParms.id == null}">value="0"</c:if> />
																			...
																			<c:when test="${fn:length(channelList) > 0}">
																				<c:forEach items="${channelList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == channelParms.channelId}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="key" name="key"
																		...
																		<c:if test="${channelParms.key != null}">value="${channelParms.key}"</c:if> />
																</div>
															</div>
														</div>
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="value" name="value"
																		class="col-xs-12 col-sm-5"
																		<c:if test="${channelParms.value != null}">value="${channelParms.value}"</c:if> />
																</div>
															</div>
														</div>
																		...
																		<c:choose>
																			<c:when test="${fn:length(statusList) > 0}">
																				<c:forEach items="${statusList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == channelParms.status}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
														...
														<button class="btn btn-sm btn-primary no-border"
															onclick="saveChannelParms()">保存</button>
													</form>
												</div>
											</div>
											<!-- /section:plugins/fuelux.wizard.container -->
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/payment/management/channelParms/saveChannelParms",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
