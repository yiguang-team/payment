...
<body class="no-skin">
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/mall/report/productReportList">
			...
			<div class="main-content">
				<div class="main-content-inner">
					...
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							<li><i class="ace-icon fa fa-home home-icon"></i> 报表管理 <span
								class="noshow jt">></span></li>
							<li class="active">商品报表</li>
						</ul>
						<!-- /.breadcrumb -->
					</div>
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									<span class="labeltxt fl">模式：</span> <select name="modelId"
										id="modelId" class="form-control fl"
										style="width: 150px; float: left;">
										<c:choose>
											<c:when test="${fn:length(modelList) > 0}">
												<c:forEach items="${modelList}" var="option">
													<option value="${option.value}"  
														<c:if test="${option.value == modelId}">selected="true"</c:if>>${option.text}</option>
												</c:forEach>
											</c:when>
											<c:otherwise>
												<option value=""></option>
									...
									<span class="labeltxt fl">开始时间：</span> <input id="beginDate"
										...
										onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'beginDate\') || \'%y-{%M-3}-%d\'}'});"
										value="${endDate}" class="fl form-text" /> 
										
										<br/><br/>
										<span class="labeltxt fl">渠道：</span> <select name="channelId"
										...
										<span class="labeltxt fl">省&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;份：</span> <select name="provinceId"
										id="provinceId" class="form-control fl"
										style="width: 150px; float: left;" onchange="changeCity();">
										<c:choose>
									...
									<span class="labeltxt fl">城&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;市：</span> <select name="cityId"
										id="cityId" class="form-control fl"
										style="width: 150px; float: left;">
										<c:choose>
									...
									</select> <br /> <br /> <span class="labeltxt fl">商户：</span> <select
										name="merchantId" id="merchantId" class="form-control fl"
										style="width: 150px; float: left;" onchange="changeProduct()">
										<c:choose>
									...
									</select> <span class="labeltxt fl">产品名称：</span> <select
										name="prodcutId" id="prodcutId" class="form-control fl"
										style="width: 150px; float: left;" onchange="changePoint()">
										<c:choose>
											<c:when test="${fn:length(prodcutList) > 0}">
												<c:forEach items="${prodcutList}" var="option">
													<option value="${option.value}"
														<c:if test="${option.value == prodcutId}">selected="true"</c:if>>${option.text}</option>
												</c:forEach>
											</c:when>
											<c:otherwise>
												<option value=""></option>
									...
									</select> <span class="labeltxt fl">计&nbsp;&nbsp;费&nbsp;&nbsp;点：</span> <select
										name="chargingPointId" id="chargingPointId"
										class="form-control fl" style="width: 150px; float: left;">
										<c:choose>
											<c:when test="${fn:length(chargingPointList) > 0}">
												<c:forEach items="${chargingPointList}" var="option">
													<option value="${option.value}"
									...
									</select> <span class="labeltxt fl">计费类型：</span> <select
