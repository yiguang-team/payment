...
<body class="no-skin">
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/logging/operationLog/showLogList">
			...
			<div class="main-content">
				<div class="main-content-inner">
					...
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							<li><i class="ace-icon fa fa-home home-icon"></i> 日志管理 <span
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									<span class="labeltxt fl">登陆名：</span> <input
										class="fl form-text" type="text" id="username" name="username"
										value="${username}"> <span class="labeltxt fl">操作对象：</span>
									<input class="fl form-text" type="text" id="operationObj"
										name="operationObj" value="${operationObj}"> <span
										class="labeltxt fl">操作类型：</span> <select name="operationType"
										id="operationType" class="form-control fl"
										style="width: 150px; float: left;">
										<c:choose>
											<c:when test="${fn:length(operationTypeList) > 0}">
												<c:forEach items="${operationTypeList}" var="option">
													<option value="${option.value}"
														<c:if test="${option.value == operationType}">selected="true"</c:if>>${option.text}</option>
												</c:forEach>
											</c:when>
											<c:otherwise>
												<option value=""></option>
													...
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="record"
															...
															<tr>
																...
																<td>${record.operationObj}</td>
																<td>${record.operationTypeLabel}</td>
																<td><fmt:formatDate value="${record.operationTime}"
																		pattern="yyyy-MM-dd HH:mm:ss" /></td>
																<td>${record.operationIp}</td>
																<td>${record.remark}</td>
															</tr>
														</c:forEach>
													</c:when>
