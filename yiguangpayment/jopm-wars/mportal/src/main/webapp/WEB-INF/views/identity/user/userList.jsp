...
<body class="no-skin">
	...
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/identity/user/userList">
			...
			<div class="main-content">
				<div class="main-content-inner">
					...
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							<li><i class="ace-icon fa fa-home home-icon"></i> 认证管理 <span
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									<span class="labeltxt fl">用户名：</span> <input
										class="fl form-text" id="form-field-icon-1" type="text"
										id="username" name="username" value="${username}"> <span
									...
									<span
										class="labeltxt fl">锁定状态：</span> <select name="isLock"
										id="isLock" class="form-control fl"
										style="width: 150px; float: left;">
										<c:choose>
											<c:when test="${fn:length(isLockList) > 0}">
												<c:forEach items="${isLockList}" var="option">
													<option value="${option.value}"
														<c:if test="${option.value == isLock}">selected="true"</c:if>>${option.text}</option>
												</c:forEach>
											</c:when>
											<c:otherwise>
												<option value=""></option>
													...
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="user"
															...
															<tr>
																...
																<td>${user.username}</td>
																<td><span
																	<c:if test="${user.status == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${user.status == 1}">class="label label-sm label-success"</c:if>>
																		${user.statusLabel} </span></td>
																<td><span
																	<c:if test="${user.isLock == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${user.isLock == 1}">class="label label-sm label-success"</c:if>>
																		${user.isLockLabel} </span></td>
																<td><div class="edit" style="float: left;">
																		<c:if test="${user.status == 0}">
																			<a href="javascript:;"
																				onclick="updateUserStatus(1,'${user.id}')">[开启]</a>
																		</c:if>
																		<c:if test="${user.status == 1}">
																			<a href="javascript:;"
																				onclick="updateUserStatus(0,'${user.id}')">[关闭]</a>
																		</c:if>
																		<a href="javascript:;"
																			onclick="editUserInfo('${user.id}')">[编辑用户信息]</a>
																		<a href="javascript:;" onclick="setRole('${user.id}')">[设置角色]</a>
...
function createUser(){
	...
	{	
		parent.addTab('新建用户','${ctx}/identity/user/createUser');
	}
...
function updateUserStatus(status,id){
	...
	bootbox.confirm("确定要"+actionStr+"此用户用户吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/identity/user/updateUserStatus",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/identity/user/deleteUser",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function editUserInfo(id){
	parent.addTab('编辑用户信息','${ctx}/identity/userInfo/editUserInfo?id='+id);
}
function setRole(id){
	parent.addTab('设置操作员角色','${ctx}/identity/user/setRole?id='+id);
}
</script>
</html>