...
<body class="no-skin">
	...
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/payment/management/channelParms/channelParmsList">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									<span class="labeltxt fl">KEY：</span> 
									<input class="fl form-text" id="form-field-icon-1" type="text"
										id="key" name="key" value="${key}"> 
										
									<span class="labeltxt fl">渠道：</span> 
									<select name="channelId"
										id="channelId" class="form-control fl"
									...
									<span class="labeltxt fl">状态：</span> <select name="status"
													...
													<div class="col-xs-6">
														<div class="btns">
															<button class="btn btn-sm btn-primary no-border"
																onclick="createChannelParms()">创建渠道参数</button>
														</div>
													</div>
													<div class="col-xs-6">
											...
											<tbody>
												<c:choose>
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="channelParms"
															varStatus="status">
															<tr>
																<td class="center">${(page-1)*pageSize+status.index+1}</td>
																<td>${channelParms.channelLabel}</td>
																<td>${channelParms.key}</td>
																<td>${channelParms.value}</td>
																<td><span
																	<c:if test="${channelParms.status == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${channelParms.status == 1}">class="label label-sm label-success"</c:if>>
																		${channelParms.statusLabel} </span></td>

																<td><div class="edit" style="float: left;">
																		<c:if test="${channelParms.status == 0}">
																			<a href="javascript:;"
																				onclick="updateChannelParmsStatus(1,'${channelParms.id}')">[开启]</a>
																		</c:if>
																		<c:if test="${channelParms.status == 1}">
																			<a href="javascript:;"
																				onclick="updateChannelParmsStatus(0,'${channelParms.id}')">[关闭]</a>
																		</c:if>
																		<a href="javascript:;"
																			onclick="editChannelParms('${channelParms.id}')">[编辑]</a>
																		<!--
								<a href="javascript:;" onclick="deleteChannelParms('${channelParms.id}')">[删除]</a>
								-->
																	</div></td>
															</tr>
														</c:forEach>
...
function createChannelParms(){
	if (parent.document.title != null)
	{	
		parent.addTab('新建渠道参数','${ctx}/payment/management/channelParms/createChannelParms');
	}

}

function updateChannelParmsStatus(status,id){
	var actionStr = "";
	if(status == 0){
		//关闭
		actionStr = "关闭";
	...
	bootbox.confirm("确定要"+actionStr+"此渠道参数吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/payment/management/channelParms/updateChannelStatus",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function deleteChannelParms(id){
	bootbox.confirm("确定要删除此渠道吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/payment/management/channelParms/deleteChannelParms",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function editChannelPaarms(id){
	parent.addTab('编辑渠道参数','${ctx}/payment/management/channelParms/editChannelParms?id='+id);
}
