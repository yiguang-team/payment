...
<body class="no-skin">
	...
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/payment/risk/merchantRejection/merchantRejectionList">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									<span class="labeltxt fl">商户：</span> <select name="merchantA"
										id="merchantA" class="form-control fl"
										style="width: 150px; float: left;">
										<c:choose>
											<c:when test="${fn:length(merchantList) > 0}">
												<c:forEach items="${merchantList}" var="option">
													<option value="${option.value}"
														<c:if test="${option.value == merchantA}">selected="true"</c:if>>${option.text}</option>
												</c:forEach>
											</c:when>
											<c:otherwise>
												<option value=""></option>
									...
									</select> <span class="labeltxt fl">互斥商户：</span> <select
										name="merchantB" id="merchantB" class="form-control fl"
										style="width: 150px; float: left;">
										<c:choose>
											<c:when test="${fn:length(merchantList) > 0}">
												<c:forEach items="${merchantList}" var="option">
													<option value="${option.value}"
														<c:if test="${option.value == merchantB}">selected="true"</c:if>>${option.text}</option>
												</c:forEach>
											</c:when>
											<c:otherwise>
												<option value=""></option>
									...
									</select> <span class="labeltxt fl">状态：</span> <select name="status"
													...
													<div class="col-xs-6">
														<div class="btns">
															<button class="btn btn-sm btn-primary no-border"
																onclick="createMerchantRejection()">创建商户互斥</button>
										...
										<table id="sample-table-1"
											...
											<thead>
												<tr>
													...
													<th class="hidden-480">商户</th>
													<th class="hidden-480">互斥商户</th>
											...
											<tbody>
												<c:choose>
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="merchantRejection"
															varStatus="status">
															<tr>
																<td class="center">${(page-1)*pageSize+status.index+1}</td>
																<td>${merchantRejection.merchantALabel}</td>
																<td>${merchantRejection.merchantBLabel}</td>
																<td><span
																	<c:if test="${merchantRejection.status == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${merchantRejection.status == 1}">class="label label-sm label-success"</c:if>>
																		${merchantRejection.statusLabel} </span></td>
																<td><div class="edit" style="float: left;">
																		<c:if test="${merchantRejection.status == 0}">
																			<a href="javascript:;"
																				onclick="updateMerchantRejectionStatus(1,${merchantRejection.id})">[开启]</a>
																		</c:if>
																		<c:if test="${merchantRejection.status == 1}">
																			<a href="javascript:;"
																				onclick="updateMerchantRejectionStatus(0,${merchantRejection.id})">[关闭]</a>
																		</c:if>
																		<a href="javascript:;"
																			onclick="editMerchantRejection('${merchantRejection.id}')">[编辑]</a>
																		<a href="javascript:;" onclick="deleteMerchantRejection(${merchantRejection.id})">[删除]</a>
																	</div></td>
															</tr>
														</c:forEach>
													</c:when>
													<c:otherwise>
														<tr>
															<td colspan="6" class="center">没数据</td>
														</tr>
													</c:otherwise>
												</c:choose>
											</tbody>
...
function createMerchantRejection(){
	if (parent.document.title != null)
	{	
		parent.addTab('新建商户互斥规则','${ctx}/payment/risk/merchantRejection/createMerchantRejection');
	}

}

function updateMerchantRejectionStatus(status,id){
	var actionStr = "";
	if(status == 0){
		//关闭
		actionStr = "关闭";
	...
	bootbox.confirm("确定要"+actionStr+"此商户互斥规则吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
...
function deleteMerchantRejection(merchantRejectionId){
...
function editMerchantRejection(id){
	parent.addTab('编辑商户互斥规则','${ctx}/payment/risk/merchantRejection/editMerchantRejection?id='+id);
}
</script>
</html>