...
<body class="no-skin">
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/depot/batch/batchList">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									<span class="labeltxt fl">批次编号：</span> <input
										class="fl form-text" type="text" id="batchId" name="batchId"
										value="${batchId}"> <span class="labeltxt fl">商户：</span>
									...
									</select> <span class="labeltxt fl">产品：</span> <select name="productId"
										id="productId" class="form-control fl"
											...
											<tbody>
												...
												<c:choose>
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="batch" varStatus="status">
															<tr>
																<td class="center">${(page-1)*pageSize+status.index+1}</td>
																<td>${batch.merchantLabel}</td>
																<td>${batch.productLabel}</td>
																<td>${batch.batchId}</td>
																<td><span
																	<c:if test="${batch.status == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${batch.status == 1}">class="label label-sm label-success"</c:if>>
																		${batch.statusLabel} </span></td>
																<td><div class="edit" style="float: left;">
																		<c:if test="${batch.status == 0}">
																			<a href="javascript:;"
																				onclick="updatePoint(1,${batch.batchId})">[上架]</a>
																		</c:if>
																		<c:if test="${batch.status == 1}">
																			<a href="javascript:;"
																				onclick="updatePoint(0,${batch.batchId})">[下架]</a>
																		</c:if>
																		<a href="javascript:;"
																			onclick="showDepotList(${batch.batchId})">[批次详细]</a>
																	</div></td>
															</tr>
														</c:forEach>
													</c:when>
...
function updatePoint(tag,batchId)
{
	var actionStr = "";
	if(tag == 0){
		//关闭
	...
	bootbox.confirm("确定要"+actionStr+"此批次商品吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/depot/batch/updateBatchStatus",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function showDepotList(batchId){
	parent.addTab('批次详细','${ctx}/depot/depot/depotList?batchId='+batchId);
}
</script>
</html>
