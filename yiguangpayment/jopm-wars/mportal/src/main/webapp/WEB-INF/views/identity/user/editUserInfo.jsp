		...
		<div class="main-content">
			<div class="main-content-inner">
				<div class="breadcrumbs" id="breadcrumbs">
					...
					<ul class="breadcrumb">
						<li><i class="ace-icon fa fa-home home-icon"></i> 用户管理 <span
												...
												<div class="step-pane active" data-step="1">
													...
													<form class="form-horizontal" id="validation-form"
														...
														<input type="hidden" name="id" id="id"
															<c:if test="${userInfo.id != null}">value="${userInfo.id}"</c:if>
															<c:if test="${userInfo.id == null}">value="0"</c:if> />
														<input type="hidden" name="uerId" id="uerId" value="${user.id}"/>
														<div class="form-group">
															<label
																class="control-label col-xs-12 col-sm-3 no-padding-right"
																for="username">用户名:</label>
															<label
																class="control-label">&nbsp;&nbsp;&nbsp;&nbsp;${user.username}</label>
														</div>
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="name" name="name"
																		...
																		<c:if test="${userInfo.name != null}">value="${userInfo.name}"</c:if> />
																</div>
															</div>
														</div>
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="email" name="email"
																		class="col-xs-12 col-sm-5"
																		<c:if test="${userInfo.email != null}">value="${userInfo.email}"</c:if> />
																</div>
															</div>
														</div>
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="mobile" name="mobile"
																		class="col-xs-12 col-sm-5"
																		<c:if test="${userInfo.mobile != null}">value="${userInfo.mobile}"</c:if> />
																</div>
															</div>
														</div>
...
<![endif]-->
	...
	<script type="text/javascript">
			...
			jQuery.validator.addMethod("unimal", function(value,element) {   
    						return /^[^·`~!@#$%^&*()_+<>?:"{},，、。’；【】.\/;'[\]]+$/.test(value);
					}, "输入值不能为空和包含其他非法字符");		
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/identity/userInfo/saveUserInfo",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
