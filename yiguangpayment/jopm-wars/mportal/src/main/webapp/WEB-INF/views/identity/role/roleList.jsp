...
<body class="no-skin">
	...
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/identity/role/roleList">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									<span class="labeltxt fl">角色名：</span> <input
										class="fl form-text" id="form-field-icon-1" type="text"
										id="roleName" name="roleName" value="${roleName}"> <span
													...
													<div class="col-xs-6">
														<div class="btns">
															<button class="btn btn-sm btn-primary no-border"
																onclick="createRole()">创建角色</button>
														</div>
													</div>
													<div class="col-xs-6">
													...
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="role"
															...
															<tr>
																...
																<td>${role.roleName}</td>
																<td><span
																	<c:if test="${role.status == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${role.status == 1}">class="label label-sm label-success"</c:if>>
																		${role.statusLabel} </span></td>
																
																<td><div class="edit" style="float: left;">
																		<c:if test="${role.status == 0}">
																			<a href="javascript:;"
																				onclick="updateRoleStatus(1,'${role.id}')">[开启]</a>
																		</c:if>
																		<c:if test="${role.status == 1}">
																			<a href="javascript:;"
																				onclick="updateRoleStatus(0,'${role.id}')">[关闭]</a>
																		</c:if>
																		<a href="javascript:;"
																			onclick="editRole('${role.id}')">[编辑]</a>
																			
																		<a href="javascript:;"
																			onclick="addTreeMenu('${role.id}')">[分配菜单]</a>
																		<a href="javascript:;"
																			onclick="addTreePrivilege('${role.id}')">[分配权限]</a>
																		<a href="javascript:;"
																			onclick="viewUserList('${role.id}')">[用户列表]</a>
																			
																		<!--<a href="javascript:;" onclick="deleteRole('${role.id}')">[删除]</a>-->
...
function createRole(){
	...
	{	
		parent.addTab('新建角色','${ctx}/identity/role/createRole');
	}

}

function updateRoleStatus(status,id){
	var actionStr = "";
	if(status == 0){
		//关闭
		actionStr = "关闭";
	...
	bootbox.confirm("确定要"+actionStr+"此角色吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/identity/role/updateRoleStatus",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/identity/role/deleteRole",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function editRole(id){
	parent.addTab('编辑角色','${ctx}/identity/role/editRole?id='+id);
}
function addTreeMenu(id){
	parent.addTab('分配菜单','${ctx}/identity/role/initRoleMenuTree?id='+id);
}
function addTreePrivilege(id){
	parent.addTab('分配权限','${ctx}/identity/role/initRolePrivilegeTree?id='+id);
}
function viewUserList(id){
	parent.addTab('用户列表','${ctx}/identity/role/viewUserList?id='+id);
}
</script>
</html>