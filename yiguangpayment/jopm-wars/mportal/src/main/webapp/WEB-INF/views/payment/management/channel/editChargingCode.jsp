...
<title>翼光联运支付管理系统-编辑渠道计费编码</title>
												...
												<div class="step-pane active" data-step="1">
													...
													<form class="form-horizontal" id="validation-form"
														...
														<input type="hidden" name="id" id="id"
															<c:if test="${chargingCode.id != null}">value="${chargingCode.id}"</c:if>
															<c:if test="${chargingCode.id == null}">value="0"</c:if> />
																			...
																			<c:when test="${fn:length(statusList) > 0}">
																				<c:forEach items="${statusList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == chargingCode.status}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
														...
														<div class="form-group">
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<select name="channelId" id="channelId" class="select2"
																		data-placeholder="请选择...">
																		<c:choose>
																			<c:when test="${fn:length(channelList) > 0}">
																				<c:forEach items="${channelList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == chargingCode.channelId}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
														...
														<div class="form-group">
															<label
																class="control-label col-xs-12 col-sm-3 no-padding-right"
																for="chargingAmount">计费值:</label>
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="chargingAmount"
																		name="chargingAmount" class="col-xs-12 col-sm-5"
																		<c:if test="${chargingCode.chargingAmount != null}">value="${chargingCode.chargingAmount}"</c:if> />
																</div>
															</div>
														</div>
														...
														<div class="form-group">
															<label
																class="control-label col-xs-12 col-sm-3 no-padding-right"
																for="chargingCode">计费编码:</label>
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="chargingCode"
																		name="chargingCode" class="col-xs-12 col-sm-5"
																		<c:if test="${chargingCode.chargingCode != null}">value="${chargingCode.chargingCode}"</c:if> />
																</div>
															</div>
														</div>
														...
														<button class="btn btn-sm btn-primary no-border"
															onclick="saveChargingCode()">保存</button>
													</form>
												</div>
											</div>
											<!-- /section:plugins/fuelux.wizard.container -->
			...
			if(result) {
				$.ajax({  
					data:{
							...
							status:$("#status").val(),
							chargingAmount:$("#chargingAmount").val(),
							chargingCode:$("#chargingCode").val(),
							
						 },
					url : "${ctx}/payment/management/channel/saveChargingCode",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
