												...
												<div class="step-pane active" data-step="1">
													...
													<form class="form-horizontal" id="validation-form"
														...
														<input type="hidden" name="id" id="id"
															<c:if test="${channel.id != null}">value="${channel.id}"</c:if>
															<c:if test="${channel.id == null}">value="0"</c:if> />
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="name" name="name"
																		...
																		<c:if test="${channel.name != null}">value="${channel.name}"</c:if> />
																</div>
															</div>
														</div>
														...
														<div class="form-group">
															<label
																class="control-label col-xs-12 col-sm-3 no-padding-right"
																for="faceAmount">开通运营商:</label>
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<c:choose>
																		<c:when
																			test="${fn:length(carrierChannelRelationList) > 0}">
																			<c:forEach items="${carrierChannelRelationList}"
																				var="option">
																				 <!--div class="checkbox" -->
																					<input name="carrierCheckbox_${option.value}"
																						id="carrierCheckbox_${option.value}" type="checkbox" class="ace"
																						value="${option.value}" onchange="upSort('carrierCheckbox_${option.value}')"
																						<c:forEach items="${checkedlist}" var="titem1"><c:if test="${option.value == titem1.carrierId}">checked</c:if>
																						</c:forEach> />
																					<span class="lbl">${option.text}</span>
																					<!--<span class="lbl">&nbsp;&nbsp;优先级:</span>-->
																					<input type="text" id="sort_${option.value}" name="sort_${option.value}" length="60" 
																						<c:forEach items="${checkedlist}" var="titem2"><c:if test="${option.value == titem2.carrierId}">value="${titem2.sort}"</c:if>
																						</c:forEach> />
																				<!-- </div>	-->
																					<br/><br/>
																			</c:forEach>
																			...
																			<c:when test="${fn:length(statusList) > 0}">
																				<c:forEach items="${statusList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == channel.status}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
...
function initSort(){
	...
	$("input[name^='carrierCheckbox_']").each(function(){ 
			var id = $(this).attr("id");
			var arr = id.split('_');
			...
			if(result) {
				$.ajax({  
					data:{
							...
							name:$("#name").val(),
							status:$("#status").val(),
							carrierChannelRelationIDs:str
						 },
					url : "${ctx}/payment/management/channel/saveChannel",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
