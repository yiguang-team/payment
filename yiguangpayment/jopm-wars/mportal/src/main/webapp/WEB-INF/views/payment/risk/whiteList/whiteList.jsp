...
<body class="no-skin">
	...
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/payment/risk/whiteList/whiteList">
													...
													<div class="col-xs-6">
														<div class="btns">
															<button class="btn btn-sm btn-primary no-border"
																onclick="createWhiteList()">创建白名单</button>
											...
											<tbody>
												<c:choose>
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="whiteList"
															varStatus="status">
															<tr>
																<td class="center">${(page-1)*pageSize+status.index+1}</td>
																<td>${whiteList.value}</td>
																<td>${whiteList.typeLabel}</td>
																<td><span
																	<c:if test="${whiteList.status == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${whiteList.status == 1}">class="label label-sm label-success"</c:if>>
																		${whiteList.statusLabel} </span></td>
																<td><div class="edit" style="float: left;">
																		<c:if test="${whiteList.status == 0}">
																			<a href="javascript:;"
																				onclick="updateWhiteListStatus(1,${whiteList.id})">[开启]</a>
																		</c:if>
																		<c:if test="${whiteList.status == 1}">
																			<a href="javascript:;"
																				onclick="updateWhiteListStatus(0,${whiteList.id})">[关闭]</a>
																		</c:if>
																		<a href="javascript:;"
																			onclick="editWhiteList('${whiteList.id}')">[编辑]</a>
																		<a href="javascript:;" onclick="deleteWhiteList(${whiteList.id})">[删除]</a>
																	</div></td>
															</tr>
														</c:forEach>
													</c:when>
...
function createWhiteList(){
	...
	{	
		parent.addTab('新建白名单规则','${ctx}/payment/risk/whiteList/createWhiteList');
	}
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/payment/risk/whiteList/updateWhiteListStatus",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/payment/risk/whiteList/deleteWhiteList",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function editWhiteList(id){
	parent.addTab('编辑白名单规则','${ctx}/payment/risk/whiteList/editWhiteList?id='+id);
}
</script>
</html>