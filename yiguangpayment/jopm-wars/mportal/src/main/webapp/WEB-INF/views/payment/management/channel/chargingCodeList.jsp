...
<body class="no-skin">
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/payment/management/channel/chargingCodeList">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									<span class="labeltxt fl">渠道名称：</span> <select name="channelId"
													...
													<div class="col-xs-6">
														<div class="btns">
															<button class="btn btn-sm btn-primary no-border"
																onclick="createChargingCode()">创建渠道计费编码</button>
													...
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="chargingCode"
															...
															<tr>
																...
																<td>${chargingCode.channelLabel}</td>
																<td><span
...
function createChargingCode(){
	...
	parent.addTab('新建渠道计费编码','${ctx}/payment/management/channel/createChargingCode');
...
function updateChargingCodeStatus(status,id){
	...
	bootbox.confirm("确定要"+actionStr+"此渠道计费编码吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/payment/management/channel/updateChargingCodeStatus",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/payment/management/channel/deleteChargingCode",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function editChargingCode(id,name){
	parent.addTab('编辑渠道计费编码-'+name,'${ctx}/payment/management/channel/editChargingCode?id='+id);
}
</script>
</html>
