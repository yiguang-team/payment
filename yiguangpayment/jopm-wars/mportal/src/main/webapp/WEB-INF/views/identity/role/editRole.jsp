												...
												<div class="step-pane active" data-step="1">
													...
													<form class="form-horizontal" id="validation-form"
														...
														<input type="hidden" name="id" id="id"
															<c:if test="${role.id != null}">value="${role.id}"</c:if>
															<c:if test="${role.id == null}">value="0"</c:if> />
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="roleName" name="roleName"
																		...
																		<c:if test="${role.roleName != null}">value="${role.roleName}"</c:if> />
																</div>
															</div>
														</div>
																			...
																			<c:when test="${fn:length(statusList) > 0}">
																				<c:forEach items="${statusList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == role.status}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/identity/role/saveRole",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
