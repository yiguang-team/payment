...
<body class="no-skin">
	<form id="listForm" name="listForm" method="post"
		action="${ctx}/payment/management/merchant/merchantList">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="table-container">
									...
									<span class="labeltxt fl">商户名称：</span> <input
									...
									<span
										class="labeltxt fl">操作员：</span> <select name="adminUser"
										id="adminUser" class="form-control fl"
										style="width: 150px; float: left;">
										<c:choose>
											<c:when test="${fn:length(userList) > 0}">
												<c:forEach items="${userList}" var="option">
													<option value="${option.value}"
														<c:if test="${option.value == adminUser}">selected="true"</c:if>>${option.text}</option>
												</c:forEach>
											</c:when>
											<c:otherwise>
												<option value=""></option>
													...
													<div class="col-xs-6">
														<div class="btns">
															<button class="btn btn-sm btn-primary no-border"
																onclick="createMerchant()">创建商户</button>
											...
											<tbody>
												...
												<c:choose>
													<c:when test="${fn:length(mlist) > 0}">
														<c:forEach items="${mlist}" var="merchant"
															varStatus="status">
															<tr>
																...
																<td>${merchant.name}</td>
																<td><span
																	<c:if test="${merchant.status == 0}">class="label label-sm label-inverse arrowed-in"</c:if>
																	<c:if test="${merchant.status == 1}">class="label label-sm label-success"</c:if>>
																		${merchant.statusLabel} </span></td>

																<td>${merchant.key}</td>
																<td>${merchant.notifyUrl}</td>
																<td>${merchant.adminUserName}</td>
																<td><div class="edit" style="float: left;">
																		<c:if test="${merchant.status == 0}">
																			<a href="javascript:;"
																				onclick="updateMerchantStatus(1,${merchant.id})">[开启]</a>
																		</c:if>
																		<c:if test="${merchant.status == 1}">
																			<a href="javascript:;"
																				onclick="updateMerchantStatus(0,${merchant.id})">[关闭]</a>
																		</c:if>
																		<a href="javascript:;"
																			onclick="editMerchant(${merchant.id},'${merchant.name}')">[编辑]</a>
																		<!--<a href="javascript:;" onclick="deleteMerchant(${merchant.id})">[删除]</a>-->
																	</div></td>
															</tr>
														</c:forEach>
													</c:when>
													<c:otherwise>
														<tr>
															<td colspan="7" class="center">没数据</td>
														</tr>
													</c:otherwise>
												</c:choose>
											</tbody>
...
function createMerchant(){
	...
	parent.addTab('编辑商户','${ctx}/payment/management/merchant/createMerchant');

}

function updateMerchantStatus(status,id){
	var actionStr = "";
	if(status == 0){
		//关闭
		actionStr = "关闭";
	...
	bootbox.confirm("确定要"+actionStr+"此商户吗?", 
		function(result) {
			if(result) {
				$.ajax({  
					data:{
					...
					url : "${ctx}/payment/management/merchant/updateMerchantStatus",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
		...
		function(result) {
			if(result) {
				$.ajax({  
					...
					url : "${ctx}/payment/management/merchant/deleteMerchant",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
...
function editMerchant(id,name){
	parent.addTab('编辑商户-'+name,'${ctx}/payment/management/merchant/editMerchant?id='+id);
}
</script>
</html>
