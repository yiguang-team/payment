												...
												<div class="step-pane active" data-step="1">
													...
													<form class="form-horizontal" id="validation-form"
														...
														<input type="hidden" id="exponent" name="exponent" class="ipt" value="${exponent}"/>
  														...
  														<input type="hidden" id="rsaPassword" class="ipt"  name="rsaPassword"/>
														
														<input type="hidden" name="id" id="id"
															<c:if test="${user.id != null}">value="${user.id}"</c:if>
															<c:if test="${user.id == null}">value="0"</c:if> />
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="text" id="username" name="username"
																		...
																		<c:if test="${user.username != null}">value="${user.username}"</c:if> />
																</div>
															</div>
														</div>
															...
															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<input type="password" id="password" name="password"
																		class="col-xs-12 col-sm-5"
																		<c:if test="${user.password != null}">value="${user.password}"</c:if> />
																</div>
															</div>
														</div>
																			...
																			<c:when test="${fn:length(statusList) > 0}">
																				<c:forEach items="${statusList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == user.status}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
														...
														<div class="form-group">
															<label
																class="control-label col-xs-12 col-sm-3 no-padding-right"
																for="isLock">锁定状态:</label>

															<div class="col-xs-12 col-sm-9">
																<div class="clearfix">
																	<select name="isLock" id="isLock" class="select2"
																		data-placeholder="请选择...">
																		<c:choose>
																			<c:when test="${fn:length(isLockList) > 0}">
																				<c:forEach items="${isLockList}" var="option">
																					<option value="${option.value}"
																						<c:if test="${option.value == user.isLock}">selected="true"</c:if>>${option.text}</option>
																				</c:forEach>
																			</c:when>
																			<c:otherwise>
																				<option value=""></option>
			...
			if(result) {
				$.ajax({  
					data:{
							...
							password:$("#rsaPassword").val(),
							status:$("#status").val(),
							isLock:$("#isLock").val()
						 },
					url : "${ctx}/identity/user/saveUser",
					type:'post',
					cache:false,
					async:true,
					dataType:'json',
